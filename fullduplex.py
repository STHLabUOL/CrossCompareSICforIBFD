# Sourced from https://github.com/abalatsoukas/fdnn
...
def plotPSD(y_test, yCanc, yCancNonLin, noise, params, type, yVar=1):
	...
	if( type == 'NL' ):
		...
		print(10*np.log10(np.var(y_test-yCanc)), 10*np.log10(np.var(yCancNonLin)))
	...
	plt.legend(['Received SI Signal ({:.1f} dBm)'.format(yTestPower), 
			 'After Linear Digital Cancellation ({:.1f} dBm)'.format(yTestLinCancPower), 
			 'After Non-Linear Digital Cancellation ({:.1f} dBm)'.format(yTestNonLinCancPower), 
			 'Measured Noise Floor ({:.1f} dBm)'.format(noisePower)], 
			 loc='upper center', bbox_to_anchor=(0.47, -0.2))
	plt.tight_layout()


	#plt.savefig('/home/nknaepper/Documents/fdnn_balatsoukas/figures/comparison/data_bal_method_bal.eps', format='eps', bbox_inches='tight')
	plt.show()

	# Return signal powers
	return fig, noisePower, yTestPower, yTestLinCancPower, yTestNonLinCancPower
